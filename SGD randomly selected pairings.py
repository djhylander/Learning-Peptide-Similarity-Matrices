...
*First iteration of the  code for this project9
@author: dhyla
"""
...
# calculates one-hot given a peptide
...
# minimizes the similarity matrix via gradient descent 
def minMatrix():  
    alpha = .01  # weight of gradient
    for i in range(100):
        gradient = findGradient()
        if (abs(gradient) > .001):  # terminate if iterations > 1000 or change in matrix < .001
...
#Basic optimization of a similarity matrix            
simMatrix = np.random.rand(20, 20)
np.set_printoptions(formatter={'float': lambda x: "{0:0.3f}".format(x)})
print("Inital random sim matrix: \n\n", simMatrix)
data = pd.read_csv("cleaned_set1.csv", usecols = ["AA_seq"])
